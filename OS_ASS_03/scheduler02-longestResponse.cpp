...
const int TRIAL_TIME_ALLOWANCE = 1; // allow to use up to this number of time slots at once for trail
        ...
        if (current_id == -1)
        {
            if (!queue_trial.empty() || !queue0.empty() || !queue1.empty()) // is anyone waiting?
            {
                if(!queue_trial.empty())
                {
                    ...
                    if (TRIAL_TIME_ALLOWANCE > customers[current_id].slots_remaining)
                    {
                        time_out = current_time + customers[current_id].slots_remaining;
                    }
                    else
                    {
                        time_out = current_time + TRIAL_TIME_ALLOWANCE;
                    }
                }
                else {
                    if (!queue0.empty()) {
