...
const int TRAIL_TIME_ALLOWANCE = 1; // allow to use up to this number of time slots at once for trail
...
void print_state(
        ...
        const std::deque<int> &queue_trial
)
{
    ...
    std::cout << '\n';
    for (int i = 0; i < queue_trial.size(); i++)
    {
        std::cout << "\t" << queue_trial[i] << ", ";
    }
    std::cout << '\n';
}
//method to sort by burst time
void sort_by_burst_time(std::deque<int> &customer_queue, std::vector<Customer> customers)
{
...
int main(int argc, char *argv[])
{
    ...
    std::deque<int> queue_trial; //every customer can have a try when they arrive
    ...
    for (int current_time = 0; !all_done; current_time++)
    {
        ...
        while (!arrival_events.empty() && (current_time == arrival_events[0].event_time))
        {
            ...
            queue_trial.push_back(arrival_events[0].customer_id);
            arrival_events.pop_front();
        }
        sort_by_priority(queue_trial, customers);
        ...
        if (current_id >= 0)
        {
            if (current_time == time_out)
            {
                ...
                if (customers[current_id].slots_remaining > 0)
                {
                    ...
                    if(customers[current_id].priority == 0){
                        ...
                        sort_by_burst_time(queue0,customers);
                    }
                    else
                    {
                        queue1.push_back(current_id);
                        sort_by_burst_time(queue1,customers);
                    }
                }
            ...
            {
                if (!queue_trial.empty())
                {
                    int last_run = current_time - customers[current_id].playing_since;
                    int temp_slots_remaining = customers[current_id].slots_remaining - last_run;
                    if(customers[queue_trial.front()].slots_remaining < temp_slots_remaining)
                    {
                        ...
                        current_id = queue_trial.front();
                        queue_trial.pop_front();
        ...
        if (current_id == -1)
        {
            if (!queue_trial.empty() || !queue0.empty() || !queue1.empty()) // is anyone waiting?
            {
                if(!queue_trial.empty())
                {
                    current_id = queue_trial.front();
        ...
        print_state(out_file, current_time, current_id, arrival_events, queue0, queue1, queue_trial);

        // exit loop when there are no new arrivals, no waiting and no playing customers
        all_done = (arrival_events.empty() && queue0.empty() && queue1.empty() && queue_trial.empty() && (current_id == -1));
    }
    return 0;
}
